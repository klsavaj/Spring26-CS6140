#!/usr/bin/env python3
"""
Sanity check script for CS6140 PS1 environment.
Verifies that the user is running in a virtual environment (venv or uv)
with required packages: numpy, matplotlib, and ipykernel.
"""
...
def load_required_packages():
    ...
    if not pyproject_path.exists():
        ...
        return ['numpy', 'matplotlib', 'ipykernel']  # Fallback to defaults
    ...
    try:
        ...
        for line in content.split('\n'):
            ...
            if in_dependencies:
                if ']' in line:
                    break
                # Extract package name from dependency string like "numpy>=2.4.0"
                match = re.match(r'\s*"([a-zA-Z0-9_-]+)', line)
                if match:
                    packages.append(match.group(1))

        return packages if packages else ['numpy', 'matplotlib', 'ipykernel']
...
def main():
    ...
    print("=" * 60)
    print("CS6140 - Environment Sanity Check")
    print("=" * 60)
    print()
    ...
    if all_checks_passed:
        ...
        print("     - pip:  pip install numpy matplotlib ipykernel")
        print("     - uv:   uv pip install numpy matplotlib ipykernel")
        return 1
